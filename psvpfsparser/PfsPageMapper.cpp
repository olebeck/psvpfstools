...
PfsPageMapper::PfsPageMapper(std::shared_ptr<ICryptoOperations> cryptops, std::shared_ptr<IF00DKeyEncryptor> iF00D, std::ostream& output, const unsigned char* klicensee, std::filesystem::path titleIdPath)
...
int PfsPageMapper::load_page_map(std::filesystem::path filepath, std::map<std::uint32_t, std::string>& pageMap) const
{
   ...
   while(std::getline(in, line))
   {
      ...
      std::uint32_t page = std::stoul(pageStr);
